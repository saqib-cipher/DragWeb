...
public class LogicBlockActivity extends AppCompatActivity implements OnClickListener, OnBlockCategorySelectListener, OnTouchListener {
		...
		private void loadLogic() {
				Map<Integer, Block> hashMap = new HashMap<>();
				...
				if (blocks != null) {
						...
						while (it.hasNext()) {
								...
								int bId = ((Integer) makeBlockFromBean.getTag()).intValue();
								hashMap.put(Integer.valueOf(bId), makeBlockFromBean);
						...
						while (it2.hasNext()) {
								...
								int beanId = safeParseInt(blockBean.id);
								Block block = (Block) hashMap.get(Integer.valueOf(beanId));
								if (block != null) {
										Block block2;
										...
										if (blockBean.parameters != null && block.args != null) {
												int size = blockBean.parameters.size();
												for (int i2 = 0; i2 < size; i2++) {
														String str = (String) blockBean.parameters.get(i2);
														if (str != null && str.length() > 0) {
																if (str.charAt(0) == '@') {
																		int refId = safeParseInt(str.substring(1));
																		block2 = (Block) hashMap.get(Integer.valueOf(refId));
																		if (block2 != null && i2 < block.args.size()) {
																				View targetArg = (View) block.args.get(i2);
																				if (targetArg instanceof BlockBase) {
																						block.replaceArgWithBlock((BlockBase) targetArg, block2);
																				}
																		}
																} else {
																		if (i2 < block.args.size()) {
																				View targetArg = (View) block.args.get(i2);
																				if (targetArg instanceof BlockArg) {
																						((BlockArg) targetArg).setArgValue(str);
						...
						// Collect all top-level root statement blocks in their original sorted order
						ArrayList<Block> rootBlocks = new ArrayList<>();
						for (Object obj : blocks) {
								if (obj instanceof BlockBean) {
										BlockBean bb = (BlockBean) obj;
										Block b = (Block) hashMap.get(Integer.valueOf(safeParseInt(bb.id)));
										if (b != null && b.parentBlock == null && !b.isReporter && b != this.pane.getRoot()) {
												rootBlocks.add(b);
										}
								}
						}

						// Stack all top-level root blocks sequentially under getRoot()
						if (!rootBlocks.isEmpty()) {
								this.pane.getRoot().insertBlock(rootBlocks.get(0));
								for (int r = 0; r < rootBlocks.size() - 1; r++) {
										rootBlocks.get(r).bottomBlock().insertBlock(rootBlocks.get(r + 1));
								}
						}
...
startActivityForResult(intent, 209);
		...
		public Block makeBlockFromBean(BlockBean blockBean) {
				...
				if (type == null || type.isEmpty() || "normal".equalsIgnoreCase(type) || "stack".equalsIgnoreCase(type) || "command".equalsIgnoreCase(type)) {
						type = " ";
				}
		...
		public void onBlockCategorySelect(int i, int i2) {
				...
				if (selectedCat.type == 0) {
						...
						java.io.File listFile = new java.io.File(FileUtil.getDragWebDir(), "collection/blocks/list.json");
						if (listFile.exists()) {
								try {
		...
		private void showSourceCode() {
				...
				builder.setTitle("Source Code Preview");

				android.widget.ScrollView scrollView = new android.widget.ScrollView(this);
				TextView codeView = new TextView(this);
				codeView.setText(finalResult);
				...
				builder.setNeutralButton("Copy", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						try {
							...
							android.content.ClipData clip = android.content.ClipData.newPlainText("Source Code", finalResult);
							clipboard.setPrimaryClip(clip);
							Toast.makeText(LogicBlockActivity.this, "Copied to clipboard", Toast.LENGTH_SHORT).show();
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
		...
		private void saveBlockToCollection(final Block block) {
				if (block == null) return;
				final java.io.File collectionFile = new java.io.File(FileUtil.getDragWebDir(), "collection/blocks/list.json");
