...
public class BlockCodeCompiler {
    ...
    public String compileBlock(BlockBean block, ArrayList<BlockBean> allBlocks, int indentLevel) {
        ...
        if (codeTemplate == null || codeTemplate.isEmpty()) {
            ...
            if (block.opCode != null && (block.opCode.startsWith("asd") || block.opCode.equals("commentBlock") || block.opCode.equals("commentBlockCss"))) {
                if (block.parameters != null && !block.parameters.isEmpty()) {
                    String raw = block.parameters.get(0);
                    if (raw != null) {
                        if (block.opCode.startsWith("commentBlock")) {
                            return "/* " + raw + " */";
                        }
                        return raw;
                    }
