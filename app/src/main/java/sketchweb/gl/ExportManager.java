...
public class ExportManager {
    ...
    public ExportResult generateExportFiles(View screen, String projectName,
        ...
        try {
            ...
            // Mirror project data files into the HTML export for backup parity
            if (projectId != null && !projectId.isEmpty()) {
                File dataDir = new File(exportDir, "data");
                if (!dataDir.exists()) dataDir.mkdirs();
                copyProjectDataToExportDir(dataDir, projectId);
            }
    ...
    private void copyProjectDataToExportDir(File dataDir, String projectId) {
        ...
        for (File file : files) {
            ...
            if (name.startsWith(projectId + ".") || name.startsWith(projectId + "_")) {
                try {
                    copyDirectory(file, new File(dataDir, name));
                } catch (IOException e) {
                    Log.w("ExportManager", "Could not copy " + name + " to export data: " + e.getMessage());
                }
            }
        }
        ...
        for (String shared : new String[]{
            ...
            if (f.exists()) {
                try {
                    copyDirectory(f, new File(dataDir, shared));
