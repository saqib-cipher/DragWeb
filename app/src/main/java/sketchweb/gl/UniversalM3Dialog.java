...
public final class UniversalM3Dialog {
     ...
     * inside a TextInputLayout-style outlined card; the "Done"/"Apply" button
     * sits on the right of the dialog footer.
     */
    public void showRadioWithCustom(List<String> presets, String currentValue, OnText cb) {
        ...
        // Custom input row: outlined input on the LEFT, the radio toggle anchors it
        ...
        customLp.topMargin = dp(4);
        customLp.bottomMargin = dp(4);
        outlined.addView(customTil, customLp);

        customEdit.setOnFocusChangeListener((v, has) -> {
            if (has) customRb.setChecked(true);
        });
        ...
        new MaterialAlertDialogBuilder(context)
            ...
            .setPositiveButton("Done", (d, w) -> {
                ...
                String result;
                if (checkedId == customId || checkedId == -1) {
