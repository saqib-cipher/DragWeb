...
public class LogicBlockManager {
    ...
    /**
     * Generate JavaScript with HTML+CSS-first approach.
     * Prefers CSS classes and HTML attributes over heavy JS manipulation.
     * Groups blocks by event+selector for cleaner output.
     */
    public String generateJavaScript() {
        if (blocks.isEmpty()) return "";

        // Group blocks by event+selector to reduce duplicate listeners
        Map<String, List<LogicBlock>> grouped = new LinkedHashMap<>();
        for (LogicBlock block : blocks) {
            String key = (block.event != null ? block.event : "immediate") + "|" + buildSelector(block);
        ...
        for (Map.Entry<String, List<LogicBlock>> entry : grouped.entrySet()) {
            List<LogicBlock> groupBlocks = entry.getValue();
            LogicBlock first = groupBlocks.get(0);
            String eventName = first.event;

            if ("immediate".equals(eventName)) {
                for (LogicBlock b : groupBlocks) {
                    js.append("  ").append(generateActionJs(b, "document.body"));
                }
                js.append("\n");
            } else if (EVENT_PAGE_LOAD.equals(eventName) || EVENT_LOAD.equals(eventName)) {
                for (LogicBlock b : groupBlocks) {
                    String el = resolveElement(b);
                    js.append("  ").append(generateActionJs(b, el));
                }
                js.append("\n");
                ...
                for (LogicBlock b : groupBlocks) {
                    js.append("      ").append(generateActionJs(b, resolveElement(b)));
                }
                js.append("    }\n  });\n\n");
                ...
                String selector = buildSelector(first);
