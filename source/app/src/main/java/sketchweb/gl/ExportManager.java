...
public class ExportManager {
    ...
    public File exportAsZip(View screen, String projectName, String projectId, LogicBlockManager logicBlockManager) throws IOException {
        ...
        try (ZipOutputStream zos = new ZipOutputStream(new BufferedOutputStream(new FileOutputStream(zipFile)))) {
            ...
            // Include external project folders (pages, styles, assets, widgets)
            includeExternalFolder(zos, projectId, "assets", "assets/");
            includeExternalFolder(zos, projectId, "pages", "pages/");
            includeExternalFolder(zos, projectId, "styles", "styles/");
            includeExternalFolder(zos, projectId, "widgets", "widgets/");
        }
        return zipFile;
    }
    ...
    /** Include an external project subfolder (pages/styles/assets/widgets). */
    private void includeExternalFolder(ZipOutputStream zos, String projectId, String folderName, String zipPrefix) {
        try {
            String path = Environment.getExternalStorageDirectory().getAbsolutePath()
                + "/.dragweb/projects/" + projectId + "/" + folderName;
            File dir = new File(path);
            if (dir.exists() && dir.isDirectory()) {
                addDirectoryToZip(zos, dir, zipPrefix);
            }
        } catch (Exception e) {
            Log.w("ExportManager", "Could not include " + folderName + ": " + e.getMessage());
        }
    }
    ...
    private String generateCss(View screen, LogicBlockManager logicBlockManager) {
        ...
        if (logicBlockManager != null) {
            css.append("\n/* Block-driven styles */\n");
            css.append(logicBlockManager.generateCssRules());
        }
