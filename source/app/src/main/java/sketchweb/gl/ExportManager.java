...
public class ExportManager {
    ...
    private void includeProjectData(ZipOutputStream zos, String projectId) {
        ...
        for (File file : files) {
            ...
            // Exclude duplicate/legacy layout.json to prevent conflicts
            if ("layout.json".equals(name) || "index.json".equals(name)) continue;

            // Map specific file types to proper folders
            if (name.startsWith(projectId + ".") || name.startsWith(projectId + "_")) {
                try {
                    String targetFolder = "widgets/";
                    if (name.endsWith(".theme")) {
                        targetFolder = "styles/";
                    } else if (name.endsWith(".meta") || name.endsWith(".json")) {
                        // Project structure goes to pages
                        targetFolder = "pages/";
                    } else if (name.endsWith(".logic")) {
                        targetFolder = "pages/";
                    }
                    addFileToZip(zos, targetFolder + name, file);
                } catch (IOException e) {
    ...
    private String generateCss(View screen, LogicBlockManager logicBlockManager) {
        ...
        if (logicBlockManager != null) {
            String logicCss = logicBlockManager.generateCSS();
