...
public class ExportManager {
    ...
    public File exportAsZip(View screen, String projectName) throws IOException {
        ExportResult result = generateExportFiles(screen, projectName);
        if (!result.success) {
    ...
    private String generateCss(View screen) {
        ...
        if (screen instanceof ViewGroup) {
            ViewGroup vg = (ViewGroup) screen;
            for (int i = 0; i < vg.getChildCount(); i++) {
                collectElementCss(vg.getChildAt(i), css);
            }
        }
    ...
    private void collectElementCss(View view, StringBuilder css) {
        if (view == null) return;
        Object tagObj = view.getTag();
        if (!(tagObj instanceof Map)) return;
