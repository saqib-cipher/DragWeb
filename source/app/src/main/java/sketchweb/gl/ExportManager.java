...
public class ExportManager {
    ...
    public ExportResult generateExportFiles(View screen, String projectName, LogicBlockManager logicBlockManager) {
        ...
        String cssContent = generateCss(screen, logicBlockManager);
        String jsContent = generateJs(logicBlockManager);
        String htmlContent = generateHtml(screen, projectName, logicBlockManager);
    ...
    private String generateHtml(View screen, String projectName, LogicBlockManager logicBlockManager) {
        ...
        if (screen instanceof ViewGroup) {
            ...
            for (int i = 0; i < vg.getChildCount(); i++) {
                html.append(generateElementHtml(vg.getChildAt(i), 1, logicBlockManager));
            }
        }
    ...
    private String generateElementHtml(View view, int indent, LogicBlockManager logicBlockManager) {
        if (view == null) return "";
        ...
        html.append(" id=\"").append(className).append("\""); // Add ID for selector

        // Custom HTML attributes from logic blocks
        if (logicBlockManager != null) {
            String attrs = logicBlockManager.generateHtmlAttributes("#" + className);
            if (attrs != null && !attrs.isEmpty()) {
                html.append(" ").append(attrs);
            }
        }
        ...
        if (view instanceof ViewGroup) {
            ...
            if (vg.getChildCount() > 0) {
                html.append("\n");
                for (int i = 0; i < vg.getChildCount(); i++) {
                    html.append(generateElementHtml(vg.getChildAt(i), indent + 1, logicBlockManager));
                }
    ...
    private String generateCss(View screen, LogicBlockManager logicBlockManager) {
        ...
        if (logicBlockManager != null) {
            String pseudoCss = logicBlockManager.generatePseudoClassCss();
            if (pseudoCss != null && !pseudoCss.isEmpty()) {
                css.append("\n/* Pseudo-classes (Logic) */\n");
                css.append(pseudoCss);
            }
        }
    ...
    private String generateJs(LogicBlockManager logicBlockManager) {
        // Pure HTML+CSS approach, no JS generated from logic blocks anymore
        return "";
    }
